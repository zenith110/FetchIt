...
func main(){
        ...
        // Grabs the latest row available
        readRange := "FE!A:V"
        ...
        if len(resp.Values) == 0 {
                ...
                for _, row := range resp.Values {
                                        ...
                                        file := FE_write(data)
                                        file_ending := strings.Replace(data.fe_term, " ", "-", -1)
                                        _ = ioutil.WriteFile("FE/" + file_ending + ".json", file, 0644)
                                        fmt.Println(data.fe_term + " is done!")
